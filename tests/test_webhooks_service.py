...
def _created_watch_result(watch_event: Mock) -> WatchEventCreateResult:
    return WatchEventCreateResult(watch_event=watch_event, created=True)
...
def test_ingest_kodi_stop_creates_watch_event(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: _created_watch_result(created_watch_event),
    )
...
def test_ingest_kodi_stop_matches_existing_watch_event(monkeypatch) -> None:
    session = Mock()
    recorded_event = Mock()
    recorded_event.playback_event_id = uuid4()
    updated_event = Mock()
    existing_movie = Mock()
    existing_movie.media_item_id = uuid4()
    existing_watch_event = Mock()
    existing_watch_event.watch_id = uuid4()
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: WatchEventCreateResult(
            watch_event=existing_watch_event,
            created=False,
    ...
    assert result.action == "duplicate_watch_event_skipped"
    assert result.watch_event is existing_watch_event
    assert result.reason == "Matched existing watch event by collision window"
